...
import { validationFieldPropietario } from './utils/validationFieldPropietario';
import { toAbsoluteUrl } from '@/utils';
import { AsignacionProcesoDocumentoInterface } from './models/DocumentosAfiliacionInterface';
...
const ModalCreatePropietario = ({ open, onClose, data, onSave, totalPorcentaje }: ModalProps) => {
    ...
    if (open) {
      if (data) {
        setFormDataPersona({
          ...
          emailExtra: data.emailExtra || '',
          celularExtra: data.celularExtra || '',
          tipoTitular: data.tipoTitular || '',
          departamentoU: data.departamentoU || '',
          idciudadUbicacion: data.idciudadUbicacion || ''
        });
        setSelectedFilePersona(data.foto || null);
      } else {
        setFormDataPersona({
          id: undefined,
          nombre1: '',
  ...
  useEffect(() => {
    if (data?.departamento) {
      fetchCiudades(data.departamento);
    }
  }, [data?.departamento]);
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <ModalContent className="max-w-[980px] top-[5%] p-4">
        <ModalHeader>
          <ModalTitle>{data ? 'Editar Propietario' : 'Nuevo Propietario'}</ModalTitle>
