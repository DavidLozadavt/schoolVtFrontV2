...
const ModalDetailPropietario = ({ open, data, onClose, onSave }: ModalProps) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(`get_documents_by_propietario/${data?.id}`);
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <ModalContent className="max-w-[680px] top-[5%] p-4">
        <ModalHeader>
          <ModalTitle>Propietario </ModalTitle>
        ...
        <ModalBody className="grid gap-3 px-0 py-2">
            ...
            <div className="card-body p-4">
              {data?.restricciones?.length > 0 && (
                <div className="px-4 py-3">
                  <h3 className="text-sm font-semibold text-gray-800 mb-2">Observaciones</h3>
          ...
          <ModalUpdateDocumentPropietario
            open={modalOpen}
            onClose={() => {
              setModalOpen(false);
            }}
