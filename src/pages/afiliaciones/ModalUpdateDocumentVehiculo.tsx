...
const ModalUpdateDocumentVehiculo = ({ open, onClose, documento, onSave }: ModalProps) => {
  const [file, setFile] = useState<File | null>(null);
  const [fechaVigencia, setFechaVigencia] = useState<string>(''); // 👈 estado para la fecha
  ...
  const validateFields = () => {
    ...
    if (!fechaVigencia) {
      newErrors.fechaVigencia = 'Debe seleccionar una fecha de vigencia.';
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.post(`update_documento_vehiculo`, data);
...
export { ModalUpdateDocumentVehiculo };
