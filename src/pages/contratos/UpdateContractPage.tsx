...
import { ContratoInterface } from '../contratacion/model/ContratoInterface';
import { validateUbicacionField } from '../contratacion/utils/validationUbicacion';
import { validateFieldPersona } from '../contratacion/utils/validationPersona';
import { validateContratoField } from '../contratacion/utils/validationContrato';
import { BancoModal } from '../contratacion/BancoModal';
...
const UpdateContractPage = ({ open, onClose, onSave }: ModalProps) => {
  ...
  useEffect(() => {
    const cargarDatosPersona = async () => {
      if (!contrato?.persona) return;

      const p = contrato.persona;

      if (p.ciudad_nac?.iddepartamento) {
        await fetchCiudades(p.ciudad_nac.iddepartamento);
      }
      if (p.ciudad_ubicacion?.iddepartamento) {
        await fetchCiudadesUbicacion(p.ciudad_ubicacion.iddepartamento);
      }
      ...
      setFormDataPersona({
        ...
        sexo: p.sexo || '',
        fechaNac: p.fechaNac || '',
        idciudadNac: p.idCiudadNac?.toString() || '',
        departamento: p.ciudad_nac?.iddepartamento?.toString() || ''
      });

      setFormDataUbicacion({
        departamentoU: p.ciudad_ubicacion?.iddepartamento?.toString() || '',
        idciudadUbicacion: p.idCiudadUbicacion?.toString() || '',
        email: p.email || '',
      ...
      setFormDataContrato({
        ...
        rol: contrato?.salario?.rol?.id?.toString() || '',
        idPension: contrato.idPension?.toString() || '',
        idSalud: contrato.idSalud?.toString() || '',
        idArl: contrato.idArl?.toString() || '',
        idCajaCompensacion: contrato.idCajaCompensacion?.toString() || '',
        idCesantias: contrato.idCesantias?.toString() || '',
        tipoCuentaBancaria: contrato.tipoCuentaBancaria || '',
        idBanco: contrato.idBanco?.toString() || '',
        numeroCuentaBancaria: contrato.numeroCuentaBancaria || '',
        observacionPreocupacional: contrato.observacionPreocupacional || '',
        idArea: contrato.idArea?.toString() || '',
        idTipoCotizante: contrato.idTipoCotizante?.toString() || '',
        idSubTipoCotizante: contrato.idSubTipoCotizante?.toString() || '',
        tipoComisiones: contrato.tipoComisiones || '',
        idActividadRiesgo: contrato.idActividadRiesgo?.toString() || '',
        idTarifaRiesgo: contrato.idTarifaRiesgo?.toString() || '',
        tipoSalario: contrato.tipoSalario || '',
        idGrupoNomina: contrato.idGrupoNomina?.toString() || ''
      });

      if (contrato.salario?.valor && contrato.fechaContratacion && contrato.fechaFinalContrato) {
        const total = calcularValorTotalContrato(
        ...
        setFormDataContrato((prev) => ({
          ...
          valorTotalContrato: total.toString()
        }));
      }

      if (contrato?.tipoContrato?.nombreTipoContrato) {
        fetchDocumentosContrato(contrato.tipoContrato.nombreTipoContrato);
      }

      if (p.rutaFotoUrl) {
        setFotoUrl(p.rutaFotoUrl);
      }

      const map: any = {};

      contrato.documentosContrato?.forEach((doc: any) => {
        map[doc.idAsignacionTipoDocumentoProceso] = doc;
      });

      setDocumentosCargados(map);
    };
  ...
  const handleChangeFormContrato = (e: any) => {
    ...
    setFormDataContrato((prevState) => {
      ...
      if (name === 'idtipoContrato' && parseInt(value) === 6) {
        updated.fechaFinalContrato = '';
        updated.valorTotalContrato = '';
      ...
      if (name === 'rol') {
        ...
        const salarioMensual = selectedRol?.salario?.valor || 0;
        const idSalario = selectedRol?.salario?.id || 0;

        updated.sueldo = salarioMensual.toString();
        updated.salario_id = idSalario;

        if (updated.fechaContratacion && updated.fechaFinalContrato) {
          const total = calcularValorTotalContrato(
            updated.fechaContratacion,
            updated.fechaFinalContrato,
            salarioMensual
          );
          updated.valorTotalContrato = total.toString();
        }
      }

      if (['fechaContratacion', 'fechaFinalContrato'].includes(name)) {
        const salarioMensual = Number(updated.sueldo || 0);

        if (updated.fechaContratacion && updated.fechaFinalContrato && salarioMensual > 0) {
          const total = calcularValorTotalContrato(
            updated.fechaContratacion,
            updated.fechaFinalContrato,
            salarioMensual
  ...
  const parseDate1 = (str: string) => {
    const [y, m, d] = str.split('-').map(Number);
    return new Date(y, m - 1, d); // <- evita UTC
  };
  ...
  const handleSaveContrato = () => {
    // Los documentos ahora son opcionales, no se valida que todos estén presentes
    setLoading(true);
    ...
    data.append('email', (formDataUbicacion.email || '').trim());
    ...
    axios
      .post(`update_contrato_persona/${contrato.idpersona}`, data)
      .then((response) => {
        const contratoData = {
          ...formDataContrato,
          fechaContratacion:
        ...
        axios
          .post(`update_contrato/${contrato.id}`, { ...contratoData })
          .then((response) => {
            ...
            const documentRequests = tipoDocumentos.map((tipoId) => {
              ...
              const existente = documentosCargados[tipoId];

              const form = new FormData();
              form.append('idContrato', idContrato + '');
              form.append('idAsignacionTipoDocumentoProceso', tipoId);

              if (file) {
                form.append('rutaFile', file);
              } else if (existente) {
                form.append('id', existente.id);
              }

              return axios
                .post('update_contrato_documento', form)
                .then(() => successCount++)
                .catch((error) => {
                  errorOccurred = true;
                  console.error(`Error al guardar documento ${tipoId}:`, error);
                });
            });
  ...
  const handleNext = () => {
    ...
    if (currentStep === 1) {
      ...
      const hasFoto = selectedFilePersona || fotoUrl || contrato?.persona?.rutaFotoUrl;
  ...
  const handleFileDelete = (id: number) => {
    ...
    setDocumentosCargados((prevDocs: any) => {
    ...
    <Fragment>
      <Container>
        <Modal open={open} onClose={onClose}>
          <ModalContent className="w-full max-w-none p-4 top-[5%]">
            <ModalHeader>
              <ModalTitle>Actualizar Contrato</ModalTitle>
              <button
                        ...
                        <form>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-2">
                            ...
                            <div>
                              ...
                              {errors.rh && (
                                <p className="text-red-500 text-sm mt-1">{errors.rh}</p>
                              )}
                            </div>
                            <div>
                              <label className="block text-sm font-medium mb-2">
                      ...
                      <div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-2">
                          {documentosContratos.map((documento) => {
                            const existente = documentosCargados[documento.id];
                            const archivoSeleccionado = selectedFiles[documento.id];
                            ...
                            return (
                              <div key={documento.id} className="mb-1">
                                ...
                                {/* SI YA EXISTE UN DOCUMENTO */}
                                {existente && !archivoSeleccionado ? (
                                  <div className="flex items-center border rounded p-2">
                                    <a
                                      href={existente.rutaFileUrl}
                                      target="_blank"
