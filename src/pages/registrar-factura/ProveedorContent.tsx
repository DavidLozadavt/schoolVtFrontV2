...
const ProvedorContent = ({ reload }: ContentProps) => {
  ...
  const handleTercero = useCallback(
    (tercero: TerceroInterface) => {
      navigate(`/factura/proveedor/registrar-factura`, { state: tercero });
    },
    [navigate]
  );
  ...
  const columns = useMemo<ColumnDef<any>[]>(
    () => [
      ...
      {
        accessorFn: (row) => (row as any).tipoTercero?.nombreTipoTercero || '—',
        id: 'tipoTercero',
        header: () => 'Tipo de Tercero',
        enableSorting: true,
        cell: (info) => (
          <span className="text-gray-700">
            {(info.row.original as any).tipoTercero?.nombreTipoTercero || '—'}
          </span>
        ),
        meta: {
          className: 'min-w-[150px]',
  ...
  const fetchTerceros = async () => {
    setLoading(true);
    try {
      const response = await axios.get('terceros');
