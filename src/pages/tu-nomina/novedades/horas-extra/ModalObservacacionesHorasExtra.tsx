...
const ModalObservacacionesHorasExtra = ({ open, onClose, data, onSave }: ModalProps) => {
  ...
  const fetchObservaciones = useCallback(async () => {
    setLoading(true);
    try {
      const response = await axios.get(`comments_solicitud_hora_extra?idSolicitud=${data?.id}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.post('store_comment_solicitud_hora_extra', payload);
...
export { ModalObservacacionesHorasExtra };
