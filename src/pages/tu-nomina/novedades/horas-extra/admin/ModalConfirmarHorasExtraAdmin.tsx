...
const ModalConfirmarHorasExtraAdmin = ({ open, onClose, data, onSave }: ModalProps) => {
  ...
  const handleSave = async (estado: string) => {
    ...
    try {
      if (data?.id) {
        await axios.put(`update_status_hora_extra_supervisor/${data?.id}`, payload);
...
export { ModalConfirmarHorasExtraAdmin };
