...
import { CommentsDer } from '../vacaciones/blocks/CommentsDer';
import { CommentsIzq } from '../vacaciones/blocks/CommentsIzq';
...
const ModalObservacacionesLicencias = ({ open, onClose, data, onSave }: ModalProps) => {
  ...
  const fetchObservaciones = useCallback(async () => {
    setLoading(true);
    try {
      const response = await axios.get(`observacion_inc_personas?idSolicitud=${data?.id}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.post('observacion_inc_personas', payload);
...
export { ModalObservacacionesLicencias };
