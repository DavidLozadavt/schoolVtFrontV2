...
const ModalConfirmarDeduccion = ({ open, onClose, data, onSave }: ModalProps) => {
  ...
  const handleSave = async (estado: string) => {
    ...
    try {
      if (data?.id) {
        await axios.put(`update_status_deducciones/${data?.id}`, payload);
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <ModalContent className="max-w-[600px] top-[5%] p-4">
        <ModalHeader>
          <ModalTitle>Finalizar Proceso</ModalTitle>
...
export { ModalConfirmarDeduccion };
