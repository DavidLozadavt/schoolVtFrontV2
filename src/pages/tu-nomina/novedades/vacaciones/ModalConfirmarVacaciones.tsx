...
const ModalConfirmarVacaciones = ({ open, onClose, data, onSave, solicitud }: ModalProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (solicitud?.id) {
        await axios.post(`update_solicitud_by_worker/${solicitud?.id}`, payload);
        enqueueSnackbar('Solicitud actualizada con éxito.', { variant: 'success' });
      } else {
        await axios.post('create_solicitud_vacaciones', payload);
...
export { ModalConfirmarVacaciones };
