...
const ModalCommentsVacaciones = ({ open, onClose, data, onSave }: ModalProps) => {
  ...
  const handleSave = async (estado: string) => {
    ...
    try {
      if (data?.id) {
        await axios.post(`update_solicitud_by_supervisor/${data?.id}`, payload);
...
export { ModalCommentsVacaciones };
